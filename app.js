...
const menuListDOM = document.querySelector(".section-center");
...
function btnFunc() {
  let allBtn = `
    ...
    `;
  btnContainerDOM.innerHTML = allBtn;
}
...
const getFood = () => {
  let allFoods = "";
  menu.map((item) => {
    allFoods += postFood(item);
  });
  menuListDOM.innerHTML = allFoods;
};
...
const getKorean = () => {
  ...
  menu.map((item) => {
    if (item.category === "Korea") {
      koreanfood += postFood(item);
    }
  });
  menuListDOM.innerHTML = koreanfood;
};
...
const getjapan = () => {
  ...
  menu.map((item) => {
    if (item.category === "Japan") {
      japanfood += postFood(item);
    }
  });
  menuListDOM.innerHTML = japanfood;
};
...
const getchina = () => {
  ...
  menu.map((item) => {
    if (item.category === "China") {
      chinafood += postFood(item);
    }
  });
  menuListDOM.innerHTML = chinafood;
};
...
allBtn.addEventListener("click", getFood);
koreaBtn.addEventListener("click", getKorean);
